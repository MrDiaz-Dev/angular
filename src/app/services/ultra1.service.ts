...
export class Ultra1Service {
  private API_URL = environment.API_URL;

  constructor(private http: HttpClient) {}

  getUltra1FromPage(page: number): Observable<UltraRespuesta> {
    const url =
      this.API_URL + `/ultra1s?page=${page}&itemsPerPage=10&pagination=true`;
    return this.http.get<UltraRespuesta>(url);
  }

  getUltra1ByID(id: number): Observable<Ultra> {
    const url = this.API_URL + `/ultra1s/${id}`;
  ...
  getUltra1By(
    ...
    const url =
      this.API_URL +
      `/ultra1s?page=${page}&itemsPerPage=10&pagination=true${toSearch}`;

    return this.http.get<UltraRespuesta>(url);
  }

  create(newUltra1: UltraCreateDTO): Observable<UltraPostRespuesta> {
    const url = this.API_URL + `/ultra1s`;
    return this.http.post<UltraPostRespuesta>(url, newUltra1);
  }
